...
/* ── BACK BUTTON (prevents leaving site) ─────── */
...
/* ── DECREMENT STOCK IN SHEETDB ───────────────── */
async function decrementStock(productId, qty) {
  if (!USE_SHEETDB) return;
  const product = products.find(p => p.id === productId);
  if (!product) return;
  const newStock = Math.max(0, product.stock - qty);
  try {
    await fetch(`${SHEETDB_URL}/id/${productId}`, {
      method: 'PATCH',
...
function stockBadgeHTML(product) {
  if (product.stock === 0)  return `<span class="stock-badge stock-out">Sold out</span>`;
  if (product.stock <= 3)   return `<span class="stock-badge stock-low">Only ${product.stock} left</span>`;
  return `<span class="stock-badge stock-ok">${product.stock} in stock</span>`;
}
...
function renderGrid(list) {
  ...
  if (!list.length) {
    grid.innerHTML = `<p style="grid-column:span 2;text-align:center;
      color:rgba(0,0,0,0.3);padding:40px 0;font-weight:700;">
      No products here yet</p>`;
    return;
  }
  ...
  list.forEach((p, i) => {
    ...
    tile.innerHTML = `
      ...
      ${soldOut ? '<div class="tile-sold-overlay">Sold Out</div>' : ''}
    `;

    if (!soldOut) tile.addEventListener('click', () => openDetail(p));
...
function initCarousel() {
  ...
  slides.forEach(slide => {
    slide.addEventListener('click', () => {
      ...
      if (product && product.stock > 0) openDetail(product);
    });
  });
...
function openDetail(product) {
  ...
  const stockWarnHTML = product.stock === 0
    ? `<p class="low-stock-warn" style="color:#E70C6A;">✕ This item is sold out</p>`
    : product.stock <= 3
  ...
  content.innerHTML = `
    ...
    <div class="detail-body">
      ...
      <div class="qty-row">
        ...
        <button class="qty-btn" onclick="changeQty(1, ${product.stock})">+</button>
      </div>` : ''}
    </div>
    <div class="cta-row">
      <button class="btn-add" onclick="addToCart()"
        ${soldOut ? 'disabled style="opacity:0.35;cursor:not-allowed;"' : ''}>
        Add to Bag
      </button>
...
function closeDetailNoHistory() {
  ...
  currentProduct = null;
  startAutoplay(document.querySelectorAll('.carousel-slide').length);
}
...
function selectVariant(btn) {
  btn.closest('.variant-options').querySelectorAll('.variant-opt')
...
function changeQty(delta, maxStock) {
  const max  = maxStock !== undefined ? maxStock : 99;
...
function addToCart() {
  ...
  if (existing) {
    existing.qty = Math.min(maxStock, existing.qty + currentQty);
  } else {
    cart.push({
      key,
      ...
      qty:      Math.min(maxStock, currentQty),
      variants,
    });
  }
...
function orderNow() {
  ...
  openWhatsApp(msg);
  decrementStock(currentProduct.id, currentQty);
}
